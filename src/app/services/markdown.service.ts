...
import { InstallSectionOptions } from '../models/install-section-options.model';
...
export class MarkdownService {
  ...
  test() {
    ...
    const githubOwner = 'hazeliscoding';
    ...
    const customBadge = this.generateBadge(
      'Custom Badge',
      'https://example.com',
      'brightgreen',
      'github',
    ...
    const contributionSection = {
      ...
      contributionGuidelinesLink:
        'https://github.com/johndoe/my-awesome-repo/blob/main/CONTRIBUTING.md',
    };
    ...
    const sections = [
      this.generateIntroductionSection(
        'My package',
        'Test package introduction just for testing',
        'https//:url.com',
    ...
    return result;
    const markdownContent = `${this.generateIntroductionSection(
      'My package',
      'Test package introduction just for testing',
      'https//:url.com',
      'https//:url.com'
...
${this.generateTableOfContentsFromMarkdown(
  `${this.generateDescription(descriptions)}${this.generateImages(
    images
...
`;
  ...
  generateTableOfContentsFromMarkdown(markdownText: string): string {
    ...
    return markdownText.replace(
      this.TABLE_CONTENT_PLACEHOLDER,
      tableOfContents
    );
  }
...
This project was created by ${authorName}. Connect with me on [GitHub](https://github.com/${githubUsername}) ${
      linkedinUsername
        ? `and [LinkedIn](https://www.linkedin.com/in/${linkedinUsername}/)`
        : ''
    } to learn more about my projects and professional background.
`;
...
5. Open a pull request against the main repository, describing the changes you made and why they should be merged.\n`;
  ...
  generateLicenseSection(licenseSection: {
    ...
    switch (type) {
      ...
      default:
        licenseSectionContent +=
          'This project is licensed under an unspecified license.\n';
    }
...
`;
  ...
  identifyTechnologies(
    description: string,
    technologies: { name: string; value: string }[]
  ): string[] {
    const foundTechnologies: string[] = [];

    technologies.forEach((tech) => {
      if (
        new RegExp(`\\b(${tech.name}|${tech.value})\\b`, 'i').test(description)
      ) {
  ...
  generateTechStackSection(
    technologies: { name: string; description: string; value: string }[]
  ) {
  ...
  generateInstallSection(options: InstallSectionOptions): string {
    ...
    if (installationSteps.length > 0) {
      installSection += `${
        dependencies.length > 0 || devDependencies.length > 0 ? '3. ' : '1. '
      }Additional steps:\n\n`;
